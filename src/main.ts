...
// Same features for touch devices
// zoom in and out feature using pinch
let lastDistance = 0;
...
canvas.addEventListener('touchmove', (e) => {
  if (touchDown) {
    const xDiff = lastTouchX - e.touches[0].clientX;
    const yDiff = lastTouchY - e.touches[0].clientY;
    origin.x -= xDiff;
    origin.y -= yDiff;
    maxX += xDiff;
    minX += xDiff;
...
canvas.addEventListener('touchstart', (e) => {
  if (e.touches.length === 1) {
    touchMove = true;
    lastTouchMoveX = e.touches[0].clientX;
...
canvas.addEventListener('touchmove', (e) => {
  if (touchMove) {
    const xDiff = lastTouchMoveX - e.touches[0].clientX;
    const yDiff = lastTouchMoveY - e.touches[0].clientY;
    origin.x -= xDiff;
    origin.y -= yDiff;
    maxX += xDiff;
    minX += xDiff;
